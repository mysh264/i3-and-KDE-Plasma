...
#ID=$(xdotool search --all --class "plasmashell" search --name "Togglehidepanelplasma" | head -n 1)
...
# If it is currently a MENU (Ghost), then check and repair
if xprop -id "$ID" _NET_WM_WINDOW_TYPE | grep -q "_NET_WM_WINDOW_TYPE_DOCK"; then
